...
def generate_test_video(output_filename="ghost_text_output.mp4", duration_seconds=5):
    # Initialize animator at 60 FPS
    animator = NoiseAnimator(width=960, height=540, fps=60)
    animator.direction = "horizontal"  # Set to horizontal shift
    animator.refresh_noise()

    # Define video writer
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_filename, fourcc, animator.fps, (animator.width, animator.height))

    total_frames = animator.fps * duration_seconds
    bg_offset = 0
    fg_offset = 0

    print(f"Generating {duration_seconds}s test video...")
    for frame_idx in range(total_frames):
        # 1. Create a dynamic text mask moving or sitting in the center
        # For a simple test, we will keep the text "GHOST" stationary in the center
        mask_data = animator.create_text_mask("GHOST", (animator.width // 2, animator.height // 2), font_size=150)

        # 2. Increment offsets in opposite directions to create opposing motion
        bg_offset += animator.animation_speed   # Moves right
        fg_offset -= animator.animation_speed   # Moves left

        # 3. Render the frame
        frame = animator.animate_frame_vectorized(mask_data, bg_offset, fg_offset)

        # Write to file
        out.write(frame)

    out.release()
    print(f"Success! Test video saved as: {output_filename}")
